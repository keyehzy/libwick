  ...
  for (size_t i = 0; i < m_nx; i++) {
    for (size_t j = 0; j < m_ny; j++) {
      for (Operator::Spin spin : {Operator::Spin::UP, Operator::Spin::DOWN}) {
        ...
        result.push_back(
            Term::one_body(-m_t, spin, index(i, j), spin, index(dsti, j)));
        result.push_back(
            Term::one_body(-m_t, spin, index(dsti, j), spin, index(i, j)));
        result.push_back(
            Term::one_body(-m_t, spin, index(i, j), spin, index(i, dstj)));
        result.push_back(
            Term::one_body(-m_t, spin, index(i, dstj), spin, index(i, j)));
      }
    }
  }
}
